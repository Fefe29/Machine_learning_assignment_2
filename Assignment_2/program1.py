...
def train_evaluate_linear_regression(X_train, X_val, X_test, y_train, y_val, y_test):
    ...
    # Train on (Train + Validation) and Test on Test
    ...
    # Predictions
    y_train_pred = model.predict(X_train_full)  # Updated to use combined train set
    y_test_pred = model.predict(X_test)  # Test remains the same

    print("\nValidation Approach Results (Train + Validation → Train, Test → Test):")
    rse_train, r2_train = calculate_metrics(y_train_full, y_train_pred, "Train + Validation")
    rse_test, r2_test = calculate_metrics(y_test, y_test_pred, "Test")

    return model, (rse_train, r2_train, rse_test, r2_test)
